...
>**누리달**은 이러한 문제를 해결하고자 **AI 기반 산전 관리 시스템**을 개발하여,  
...
- **에이전트** - 캘린더 연동 서비스 구현
    ...
    - 이를 통해 캘린더 탭에서 LLM 대화, 태교 일기, 일정 관리 등 핵심 기능에 대한 통합적인 관리가 가능해짐

---

### (2) 아쉬운 점
- **에이전트 정보 제공 한계**
    - 거주 지역을 기준으로 주변의 분만실 보유 산부인과 혹은 난임전문병원을 추천하고자 했으나, AI 에이전트가 최신 정보를 제대로 반영하지 못하여서 기대한 만큼의 성능을 얻지 못함
- **달력 UX 완성도 개선 필요**
    - 반복 일정을 화면에 렌더링하는 과정에서 약간의 시간 지연이 발생함
    - 일정 표시와 관련된 UI 개선 필요해보임
- **알림 시스템 미구현**
...
### (3) 향후 계획
- **정확한 최신 정보 제공 체계 구축**
    - 정부 기관과의 협업을 통해 정부 기관이 보유한 데이터를 활용하여 정확한 정책/병원 정보를 제공하는 시스템 구축
    - 혹은 네이버맵/카카오맵 등의 국내 지도 API를 활용하여 에이전트가 병원 관련 최신 데이터를 검색할 수 있는 외부 데이터 연동 시스템 개발
    - 의료 전문가 검수 시스템을 도입하여 에이전트가 제공하는 건강 정보의 신뢰성 향상
- **추가 기능 구현**
    - **달력 UX 개선**
    ...
    - **알림 시스템 구축**
        ...
        - 주요 일정에 대한 리마인더 기능
    - **소셜 기능 확장**
        - 팔로잉/팔로워 기능을 고도화하여 배우자/가족과의 일정 공유 및 협업 서비스 제공