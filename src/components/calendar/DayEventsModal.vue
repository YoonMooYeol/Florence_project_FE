...
<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center day-events-modal p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-auto overflow-hidden">
      ...
      <div class="bg-point px-6 py-4 flex justify-between items-center">
        <h3 class="text-lg font-bold text-dark-gray">{{ date ? formatDate(date) : '' }}</h3>
      ...
      <div class="p-6 bg-ivory">
        ...
        <div>
          ...
          <div v-if="!llmSummary" class="text-center py-4 text-gray-500">
            이 날짜에 대화 내용이 없습니다.
          </div>
          <div 
            v-else 
          ...
          >
            ...
            <p class="text-sm text-gray-600">{{ llmSummary.summary }}</p>
          </div>
        </div>
      </div>
